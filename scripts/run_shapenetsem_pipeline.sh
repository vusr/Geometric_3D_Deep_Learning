...
python scripts/download_shapenetsem.py \
  --data_dir "$DATA_DIR" \
...
python scripts/prepare_shapenetsem_data.py \
  --data_dir "$DATA_DIR" \
  --n_points 1024 \
  --workers 2 \
  --limit "$SMOKE_LIMIT" \
...
python scripts/analyze_shapenetsem_targets.py \
  --data_dir "$DATA_DIR" \
  --seed 42 \
...
tar -czf "$RUN_DIR/shapenetsem_regression_artifacts.tgz" \
  checkpoints/shapenetsem_regression \
  logs/shapenetsem_regression \
  results/shapenetsem_regression \
  data/shapenetsem_regression/splits.csv \
  data/shapenetsem_regression/norm_stats.json \
  data/shapenetsem_regression/analysis_summary.json \
  data/shapenetsem_regression/outliers_removed.csv \
  data/shapenetsem_regression/analysis_plots || true
