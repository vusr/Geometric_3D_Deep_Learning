...
def evaluate_one(args, model_name: str, cluster_id: int | None, device):
    ...
    checkpoint = (
        ...
        else run_dir / "artifacts" / "modelnet40_regression" / "checkpoints" / model_name / "best.pth"
    )
    ...
    results_dir = Path(args.results_dir) if args.results_dir else run_dir / "artifacts" / "modelnet40_regression" / "results" / model_name
...
def main():
    ...
    if mode == "global":
        ...
        summary_path = Path(args.run_dir) / "artifacts" / "modelnet40_regression" / "results" / "oracle_cluster_summary.json"
