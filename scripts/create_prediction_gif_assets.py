...
def create_modelnet_classification(run_dir: Path, data_dir: Path, output_root: Path) -> None:
    output_dir = output_root / "modelnet_classification" / "rotating_samples"
    ensure_dir(output_dir)
    preds = pd.read_csv(run_dir / "results" / "cls_sample_predictions.csv")
    ...
    for i, row in selected.iterrows():
        ...
        rows.append(
            {
                "gif": f"assets/modelnet_classification/rotating_samples/{gif_name}",
...
def create_modelnet_regression(run_dir: Path, data_dir: Path, output_root: Path) -> None:
    output_dir = output_root / "modelnet_regression" / "rotating_samples"
    ensure_dir(output_dir)
    ...
    for i, row in selected.iterrows():
        ...
        rows.append(
            {
                "gif": f"assets/modelnet_regression/rotating_samples/{gif_name}",
...
def create_shapenet_geometry(run_dir: Path, data_dir: Path, output_root: Path) -> None:
    output_dir = output_root / "shapenetsem_regression" / "rotating_samples"
    ensure_dir(output_dir)
    preds = pd.read_csv(run_dir / "results" / "shapenetsem_regression" / "global" / "reg_predictions.csv")
    splits = pd.read_csv(data_dir / "shapenetsem_regression" / "splits.csv")
    test_rows = splits[splits["split"] == "test"].reset_index(drop=True)
    preds = preds.copy()
    ...
    for i, row in selected.iterrows():
        ...
        rows.append(
            {
                "gif": f"assets/shapenetsem_regression/rotating_samples/{gif_name}",
...
def create_shapenet_weight(run_dir: Path, data_dir: Path, output_root: Path) -> None:
    output_dir = output_root / "shapenetsem_weight" / "rotating_samples"
    ensure_dir(output_dir)
    preds = pd.read_csv(run_dir / "results" / "shapenetsem_weight" / "auxiliary" / "weight_predictions.csv")
    splits = pd.read_csv(data_dir / "shapenetsem_regression" / "weight_splits.csv")
    test_rows = splits[splits["split"] == "test"].reset_index(drop=True)
    preds = preds.copy()
    ...
    for i, row in selected.iterrows():
        ...
        rows.append(
            {
                "gif": f"assets/shapenetsem_weight/rotating_samples/{gif_name}",
                "alt": f"Rotating ShapeNetSem {row['class_name']} point cloud",
            }
        )
...
def main() -> None:
    ...
    parser.add_argument("--output_root", default=None, help="Asset root, defaults to <run_dir>/assets")
    args = parser.parse_args()

    run_dir = Path(args.run_dir).expanduser().resolve()
    data_dir = Path(args.data_dir).expanduser().resolve() if args.data_dir else run_dir / "data"
    output_root = Path(args.output_root).expanduser().resolve() if args.output_root else run_dir / "assets"

    create_modelnet_classification(run_dir, data_dir, output_root)
    create_modelnet_regression(run_dir, data_dir, output_root)
    create_shapenet_geometry(run_dir, data_dir, output_root)
    create_shapenet_weight(run_dir, data_dir, output_root)
    ...
    (output_root / "prediction_gif_manifest.json").write_text(json.dumps(manifest, indent=2) + "\n", encoding="utf-8")
    print(f"Saved rotating prediction assets under {output_root}")
