...
def main():
    ...
    if not args.skip_prepare:
        run([
            sys.executable, str(project_dir / "tools" / "data" / "prepare_modelnet40_regression_data.py"),
            "--workers", str(args.prepare_workers),
        ])

    run([
        sys.executable, str(project_dir / "tools" / "data" / "analyze_modelnet_surface_bbox_targets.py"),
    ...
    train_common = [
        sys.executable, "-m", "experiments.modelnet40_regression.train",
        "--data_dir", str(data_dir),
    ...
    eval_common = [
        sys.executable, "-m", "experiments.modelnet40_regression.evaluate",
        "--data_dir", str(data_dir),
        "--run_dir", str(project_dir),
        "--workers", str(args.workers),
    ]
