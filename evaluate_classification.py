...
Usage:
    python evaluate_classification.py [options]

Key options:
    --data_dir      Root data directory
    --checkpoint    Path to best.pth  (default: checkpoints/cls/best.pth)
...
from src.utils.metrics import classification_metrics, measure_throughput


def collate_fn(batch):
    points_list, labels_list = zip(*batch)
    return torch.stack(points_list, dim=0), torch.stack(labels_list, dim=0)
...
def main():
    ...
    parser.add_argument("--checkpoint", default=None,
                        help="Path to checkpoint. Defaults to <run_dir>/checkpoints/cls/best.pth")
    parser.add_argument("--results_dir", default=None,
                        help="Output directory. Defaults to <run_dir>/results")
    ...
    checkpoint  = Path(args.checkpoint) if args.checkpoint else run_dir / "checkpoints" / "cls" / "best.pth"
    results_dir = Path(args.results_dir) if args.results_dir else run_dir / "results"
    ...
    # --- Save results ---
    results_txt = results_dir / "cls_test_results.txt"
    ...
    metrics_json = results_dir / "cls_metrics.json"
    metrics_payload = {
        "dataset": "ModelNet40",
    ...
    per_class_csv = results_dir / "cls_per_class_acc.csv"
    ...
    cm_csv = results_dir / "cls_confusion_matrix.csv"
    ...
    sample_csv = results_dir / "cls_sample_predictions.csv"
